...
internal data class AppAppearance(val style: AppearanceStyle = AppearanceStyle.GLASS, val compact: Boolean = false) {
    val dark get() = style == AppearanceStyle.DARK
    val glass get() = style == AppearanceStyle.GLASS
    ...
    val border get() = if (dark) Color(0xFF494750) else if (glass) Color.White.copy(alpha = .65f) else Color(0xFFE2E2EA)
    val navigation get() = if (glass) listOf(Color(0xFFF6F0FF).copy(alpha = .96f), Color(0xFFE7DFF8).copy(alpha = .88f)) else card
...
internal fun AppearancePreferences() {
    ...
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        listOf(false, true).forEach { compact ->
            AppChoiceChip(
                selected = appearance.compact == compact,
                onClick = { prefs.edit().putBoolean("compact", compact).apply() },
                label = { Text(if (compact) lt(language, "Lista compacta", "Compact list", "Lista compacta")
                    else lt(language, "Grade", "Grid", "Cuadrícula")) }
            )
        }
    }
