    ...
    fun shouldOpenReview(data: PokemonScreenData, fields: List<NamingField>): Boolean {
        return fields.any { field ->
            when (field) {
                ...
                NamingField.UNIQUE_FORM -> when {
                    isVivillonFamily(data.pokemonName) -> data.vivillonPattern == null
                    data.pokemonName.equals("Unown", ignoreCase = true) -> data.unownLetter.isNullOrBlank()
                    else -> UniquePokemonCatalog.optionsFor(data.pokemonName).isNotEmpty() && data.uniqueForm.isNullOrBlank()
                }
